...
2. 상태 조회 스킬에서 이미 확보한 Jenkins 접속 정보가 있으면 동일한 값을 재사용한다.
3. 접속 정보가 없고 `jenkins` MCP 서버도 없으면 Jenkins URL, 계정, 토큰 또는 비밀번호를 사용자 인터랙션으로 입력받아 연결한다.
4. 사용자가 요청한 잡 이름이 `dev-` 로 시작하는지 먼저 확인한다.
5. `dev-` 패턴이 아니면 즉시 거부한다.
6. 가능하면 쓰기 가능한 Jenkins MCP 툴을 사용하고, 없으면 `scripts/jenkins_build_runner.py` 로 폴백한다.

## 최초 1회 필수 규칙

1. `jenkins` MCP 서버 존재 여부를 먼저 확인한다.
2. 상태 조회 스킬이 이미 확보한 Jenkins URL, 사용자명, 토큰 또는 비밀번호가 있으면 그 값을 그대로 사용한다.
3. 이미 확보한 정보가 없고 MCP도 없으면 조회나 실행 전에 먼저 연결한다.
4. Jenkins URL, 사용자명, API 토큰 또는 비밀번호는 런타임 사용자 인터랙션으로 수집한다.
5. 저장소 파일, 스킬 본문, 예제에는 실제 시크릿을 하드코딩하지 않는다.
...
5. 상태 조회 스킬과 동일한 Jenkins 접속 정보를 우선 재사용한다.
6. MCP 쓰기 툴이 있으면 그것을 우선 사용한다.
7. 없으면 스크립트 폴백을 사용한다.
8. 실행 후에는 큐 등록 여부 또는 응답 코드를 짧게 보고한다.
...
상태 조회 스킬이 이미 같은 값을 확보했다면 동일한 값을 재사용한다.
